...
# post-comment.sh — Read test reports and post a summary as a PR comment.
# Usage: post-comment.sh <PR_NUMBER>
...
PR_NUMBER="${1:-}"
if [[ -z "${PR_NUMBER}" ]]; then
  gh_error "Usage: post-comment.sh <PR_NUMBER>"
  exit 1
fi
...
# ── No reports: post "no features affected" and exit ─────────────────────────
if [[ ${#REPORT_FILES[@]} -eq 0 ]]; then
  gh_group "Posting comment (no reports)"
  COMMENT="## Agentic Test Results

No features were affected — no test reports generated.

---
Full reports available as [workflow artifacts](../actions)."

  gh pr comment "${PR_NUMBER}" --body "${COMMENT}"
  echo "Posted 'no features affected' comment"
  gh_endgroup
  exit 0
fi
...
# ── Assemble PR comment ─────────────────────────────────────────────────────
gh_group "Assembling PR comment"
...
# ── Post the comment ─────────────────────────────────────────────────────────
gh_group "Posting PR comment"

gh pr comment "${PR_NUMBER}" --body "${COMMENT}"

echo "Posted agentic test results to PR #${PR_NUMBER}"
gh_endgroup
