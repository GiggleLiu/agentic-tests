...
# install-opencode.sh — Install OpenCode CLI for Linux amd64 (GitHub Actions runner)
# Downloads the latest release from GitHub and installs to /usr/local/bin/opencode.
...
gh_group "Resolving latest OpenCode release for linux/amd64"

API_URL="https://api.github.com/repos/${REPO}/releases/latest"

# Fetch release metadata (honour GITHUB_TOKEN when available to avoid rate limits)
...
# Find the linux amd64 asset — look for names containing linux and amd64/x86_64
ASSET_URL=""
ASSET_NAME=""

# Try common naming patterns in order of preference
for pattern in 'linux.*amd64' 'linux.*x86_64' 'Linux.*amd64' 'Linux.*x86_64'; do
...
# ── 4. Extract / install ────────────────────────────────────────────────────
gh_group "Installing opencode to ${INSTALL_DIR}"
...
if ! command -v "${BINARY_NAME}" &>/dev/null; then
  gh_error "opencode not found on PATH after installation"
  exit 1
fi
